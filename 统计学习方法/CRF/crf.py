...
class CRF(object):
    def __init__(self,feature_functions, labels, sigma=10, transition_feature=True):
        ...
        self.label_id = {l:i for i, l in enumerate(self.labels)}  # 标签和索引的对应关系
        # 这是做什么的?
        self.v = sigma**2
        self.v2 = self.v*2

    def regulariser(self, w):
        return np.sum(w ** 2) / self.v2  # 正则化
    ...
    def all_features(self, x_vec):
        ...
        for i in range(len(x_vec)+1):
            for j, yp in enumerate(self.labels):  # 状态序列,标签序列,这一个循环表示上一个状态
                for k,y in enumerate(self.labels):  # 这一个循环表示当前状态
    ...
    def forward(self, M, start=0):
        ...
        alpha = alphas[0]
        alpha[start] = 0
        for i in range(M.shape[0]-1):  # 行表示状态取值的个数
            alpha = alphas[i+1] = log_dot_vm(alpha, M[i])  # 乘以转移概率,递推公式(11,27)
    ...
    def backward(self, M, end=-1):
        ...
        for i in reversed(range(M.shape[0]-1)):
            beta = betas[i] = log_dot_mv(M[i+1], beta)  # 递推公式(11,31)
    ...
    def neg_likelihood_and_deriv(self, x_vec_list, y_vec_list, theta, debug=False):
        ...
        for x_vec, y_vec in zip(x_vec_list, y_vec_list):
            ...
            log_alphas, last = self.forward(log_M, self.label_id[START]) #前向算法
            log_betas, zero = self.backward(log_M, self.label_id[END])  # 后向算法
    ...
    def slow_predict(self, log_potential, N, K, debug=False):
        ...
        for t in xrange(1,N):
            ...
            for y in xrange(K):
                ...
                U[y] = w[b]
            V = U
        #extract the best path by brach_tracking
        y = V.argmax()
        trace = []
